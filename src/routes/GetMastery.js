...
routes.get('/mastery/:summonerName/:region', async (req, res) => {
  ...
  const masteryData = masteryResponse.data.map((champion) => {
    return {
...
routes.get('/masterySummary/:summonerName/:region', async (req, res) => {
  const { summonerName, region } = req.params;
  const summonerIdResponse = await axios
    .get(
      `https://${region}.api.riotgames.com/lol/summoner/v4/summoners/by-name/${summonerName}?api_key=${process.env.API_KEY}`,
  ...
  const championPoints = masteryResponse.data;

  let totalChampionPoints = 0;
  for (let i = 0; i < masteryResponse.data.length; i++) {
    totalChampionPoints += masteryResponse.data[i].championPoints;
  }
